<template>
  <div>
    ...
    >
      ...
      >
        <input
          id="dark-mode"
          type="checkbox"
          class="absolute dark-mode-checkbox opacity-0"
        />
      </label>
    </div>
    <nav>
        ...
        <li>
          <a
            ...
            class="
              ...
              bg-yellow-300
              dark:bg-purple-300
              hover:bg-yellow-400 hover:translate-y-1
              dark:hover:bg-purple-400
              active:bg-yellow-400 active:translate-y-2.5
              ...
              dark:before:bg-purple-600
              before:transition-shadow before:transition-transform
              transition-colors transition-transform
            "
          >
            <svg
              viewBox="0 0 412.3 172.2"
              class="w-auto h-6 md:h-8"
              xmlns="http://www.w3.org/2000/svg"
            >
              <polyline
    ...
    <div id="frame" class="w-full min-h-screen">
      <div
        id="sigil"
        class="absolute w-full h-full top-0 pointer-events-none z-20"
      ></div>
      <canvas
        id="dots"
        class="absolute w-full h-full top-0 pointer-events-none z-10"
      />
    </div>
    <Nuxt />
  </div>
    ...
    this.$nextTick(() => {
      ...
      function draw(time = 0) {
        ...
        for (let i = 0; i < dotCount; i++) {
          ...
          if (mouseDown) {
            const s =
              Math.sqrt((d.x - cX) * (d.x - cX) + (d.y - cY) * (d.y - cY)) /
                mD +
              0.2
            d.x -= (d.x - cX) * s
            d.y -= (d.y - cY) * s
            ...
            const s =
              Math.sqrt(
                (d.x - d.oX) * (d.x - d.oX) + (d.y - d.oY) * (d.y - d.oY)
              ) /
                mD +
              0.2
            d.x -= (d.x - d.oX) * s
          ...
          if (
            (Math.abs(d.x - d.oX) > f || Math.abs(d.y - d.oY) > f) &&
            !mouseDown
          ) {
            d.c += d.c < rot ? 360 : -rot

            const k = (d.c * Math.PI) / 180
            const r = (Math.sin(k) * l + v) | 0
            const g = (Math.sin(k + (2 * Math.PI) / 3) * l + v) | 0
            const b = (Math.sin(k + (4 * Math.PI) / 3) * l + v) | 0
      ...
      checkbox.checked = localStorage.getItem('darkModeST13') === 'true'
      ...
      checkbox.addEventListener('change', function (event) {
        localStorage.setItem('darkModeST13', event.currentTarget.checked)
